  ...
  const unlisten = await listen<{ id: string; action: string; input?: string; index?: number; variation?: number; enabled?: boolean }>("test-control-request", async ({ payload }) => {
    try {
      ...
      switch (payload.action) {
        ...
        case "newGame": result = await game.createNewSave(); break;
        ...
        case "cancel": result = game.cancelGeneration(); break;
        case "reload": await game.loadSaves(); result = true; break;
        case "play": await router.navigate('/play'); result = true; break;
