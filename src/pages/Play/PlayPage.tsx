    ...
    <div className="flex h-full gap-5 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 select-text">
          {activeSave && !textWorld && <div className="p-4 rounded-lg bg-amber-50 text-sm">此存档使用已退役的状态管线，历史保留。<button className="ml-3 text-blue-700" disabled={isExecuting} onClick={() => migrateActiveToText().catch(() => window.alert('迁移失败，原存档保留'))}>迁移到故事组合</button></div>}
          {turns.map((turn, idx) => (
